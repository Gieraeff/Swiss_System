...
class TournamentGUI:
    ...
    def _build_control_tab(self) -> None:
        ...
        wave_frame = ttk.LabelFrame(left, text="Aktuelle Welle / Pairing", style="Card.TLabelframe")
        wave_frame.pack(fill="both", expand=False, pady=(0, 10))
        self.wave_tree = ttk.Treeview(
            wave_frame,
        ...
        for col, title, width in [
            ...
            self.wave_tree.heading(col, text=title)
            self.wave_tree.column(col, width=width, anchor="center")
        self.wave_tree.pack(fill="x", expand=False)
        ...
        self.preview_tree = ttk.Treeview(
            preview_frame,
            columns=("idx", "slot", "team_a", "team_b", "status"),
            show="headings",
            height=3,
        )
        for col, title, width in [
    ...
    def _refresh_preview_tree(self) -> None:
        ...
        for idx, row in enumerate(rows, start=1):
            self.preview_tree.insert("", "end", values=(idx, row["slot"], row["team_a"], row["team_b"], row["status"]))
    ...
    def rebuild_preview(self) -> None:
        # The preview is derived from the fixed current wave and intentionally not recalculated every refresh.
        self.refresh_all()
